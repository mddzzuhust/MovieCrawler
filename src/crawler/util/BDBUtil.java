 ...
 * @author fugui
 *
 * Berkeley DB因无法很好的解决缓存链接删除问题（必须得一条一条地删），故暂时未用
 * 而将链接通过文件保存在本地磁盘上，可以在读取后直接删除整个文件。
 * 
 * 可尝试其他内存数据库，如Redis、memcached
 * 参见淘宝搜索技术博客：http://www.searchtb.com/2011/07/%E5%BF%AB%E9%80%9F%E6%9E%84%E5%BB%BA%E5%AE%9E%E6%97%B6%E6%8A%93%E5%8F%96%E9%9B%86%E7%BE%A4.html?spm=0.0.0.0.sPCFcA
 */
public class BDBUtil{ 
 
   public static Environment dbEnvironment = null;
...
//   }
   ...
   public List<String> getDatas2(boolean numLimit, int num){
	   System.out.println(new Timestamp(System.currentTimeMillis()));
	   List<String> list = null;
	   int count = 1;
	   Transaction txn = null;
   ...
   public void deleteDatas(List<String> keys){
	   
	   for(int i = 0; i < keys.size(); i++){
		   deleteData(keys.get(i));
	   }
	   /*
	   try{
		   DatabaseEntry theKey = new DatabaseEntry(Key.getBytes("UTF-8"));
   ...
   public static void main(String[] args){
	   ...
	   //db.findData("test");
//	   db.addData("test", "world");
//	   db.addData("hello1", "");
//	   db.addData("hello2", "");
//	   db.addData("hello3", "");
	   //db.addData("test", "just test");
	   
	   /*List<String> list2 = */db.getDatas2(true, 500);
	   /*List<String> list1 = */db.getDatas(true, 500);
	   /*
